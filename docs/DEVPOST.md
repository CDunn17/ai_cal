...
MyCP is a seeded shared-calendar demo for a distributed launch team. A browser agent uses WebMCP tools to inspect narrowly scoped context, find one-time or weekly availability, and rank meeting options. A weekly proposal checks every requested occurrence against private busy time, protected focus blocks, work patterns, travel buffers, and time preferences without revealing why another person is unavailable. When a weekly check-in conflicts with existing meetings, it can stage bounded one-off reschedules for only those instances without moving the existing events. The agent can create the series only from the selected, expiring proposal; MyCP rechecks every occurrence before review and commit.
...
MyCP registers fifteen intent-level tools rather than exposing DOM selectors or database access. `get_calendar_context` provides bounded calendar/team identifiers, while `get_user_scheduling_profile` reads a purpose-limited profile with meeting preferences, recurring focus blocks, and office/remote work patterns—never a home address, live location, or private events. `propose_recurring_schedule` validates a requested weekly local time across 2–26 occurrences and returns an expiring, single-use proposal ID with no more than four explicit, reviewable exceptions. `create_recurring_event_draft_from_proposal` is the only recurring creation path: it binds the validated time, attendees, and exceptions to that proposal, then revalidates every occurrence at draft creation and human review/commit. `get_events_in_range` and `propose_time_away_changes` return only active events the user owns, while `create_time_away_change_set_draft` can only stage—not apply—cancellations and approved-delegate transfers. Read tools are annotated as read-only. Tool outputs carrying calendar content are marked untrusted and limited to 1.4 KB of UTF-8 data; the adapter returns compact events and candidate slots, or a safe truncation response. `commit_event` is intentionally blocked so an agent cannot bypass the human confirmation flow.
