...
| `propose_time_away_changes` | No | Previews a private time-away block plus cancellation of the active user’s owned one-time meetings and individual recurring occurrences in range, with selected one-time events transferred only to an approved delegate. |
...
| `create_event_draft` | Draft only | Creates a visible, server-validated one-time draft with a 24-hour expiry. Availability is rechecked when drafted, reviewed, and committed; it cannot send invitations. |
...
| POST /api/time-away-proposals | Previews the exact owned one-time events and recurring occurrences a time-away plan would cancel or transfer. This is read-only. |
| POST /api/time-away-change-sets | Creates an expiring, reviewable change-set draft containing a private absence block, one-time/recurring-occurrence cancellations, and approved-delegate transfers. |
...
- `create_recurring_event_draft_from_proposal` is the only recurring creation path. The server binds the selected time, attendees, and exceptions to the proposal, revalidates every occurrence at draft creation and again at human review and commit, and rejects stale/conflicting series. `create_event_draft` is one-time only and revalidates current availability at creation, review, and commit; recurring draft scheduling fields cannot be edited in place.
- `create_time_away_change_set_draft` creates a visible change-set only. It stages cancellation of each owned recurring occurrence in range rather than cancelling its whole series. An agent cannot apply any cancellation, transfer, or notification. The human confirmation rechecks every affected event revision, applies the entire plan together, and records each applied change in the audit trail.
